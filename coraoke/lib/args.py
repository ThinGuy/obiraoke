...
from coraoke.lib.get_platform import get_default_dl_dir, get_platform
...
def parse_coraoke_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--headless",
        action="store_true",
        help="Headless mode. Don't launch the splash screen/player on the coraoke server",
        required=False,
    )
    parser.add_argument(
        "--logo-path",
    ...
    parser.add_argument(
        "--hide-overlay",
        action="store_true",
        help="Hide all overlays that show on top of video, including current/next song, coraoke QR code and IP",
        required=False,
    )
    parser.add_argument(
        "--hide-notifications",
    ...
    parser.add_argument(
        "--limit-user-songs-by",
        help=f"Limit the number of songs a user can add to queue. User name 'Coraoke' is always unlimited (default: {_DEFAULTS['limit_user_songs_by']} = unlimited)",
        default=None,
        required=False,
    )
    parser.add_argument(
