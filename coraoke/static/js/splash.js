...
const hasBgVideo = CoraokeConfig.hasBgVideo;
let currentVideoUrl = null;
let hlsInstance = null;
let idleTime = 0;
let screensaverTimeoutSeconds = CoraokeConfig.screensaverTimeout;
let bg_playlist = [];
...
const endSong = async (reason = null, showScore = false) => {
  if (showScore && !CoraokeConfig.disableScore) {
    isScoreShown = true;
...
const playBGMusic = async (play) => {
  ...
  if (play) {
    if (CoraokeConfig.disableBgMusic) return;
    ...
    $(audio).animate({ volume: CoraokeConfig.bgMusicVolume }, 2000);
  } else {
    if (audio) {
...
const playBGVideo = async (play) => {
  ...
  if (play) {
    if (CoraokeConfig.disableBgVideo) return;
    if (!autoplayConfirmed) return;
...
const setupOverlayMenus = () => {
  ...
  const triggerInactivity = () => {
    ...
    cursorVisible = false;
    $("#menu a").fadeOut();
    if (CoraokeConfig.showSplashClock) {
      setTimeout(() => {
...
const handleUnsupportedBrowser = () => {
  if (!isSupportedBrowser) {
    ...
    warningMessage.innerHTML =
      CoraokeConfig.translations.unsupportedBrowser;
...
const toggleBGMedia = (configKey, playFn, disabled) => {
  CoraokeConfig[configKey] = disabled;
...
const PREFERENCE_EFFECTS = {
  ...
  disable_score:       (v) => { CoraokeConfig.disableScore = v; },
  show_splash_clock:   (v) => {
    CoraokeConfig.showSplashClock = v;
    v ? startClock() : (stopClock(), $("#clock").hide());
  },
  hide_overlay:        (v) => {
    CoraokeConfig.hideOverlay = v;
    $("#bottom-container, #top-container").toggle(!v);
  },
  hide_url:            (v) => { $("#qr-code, #screensaver-qr").toggle(!v); },
  bg_music_volume:     (v) => {
    CoraokeConfig.bgMusicVolume = v;
  ...
  screensaver_timeout: (v) => {
    ...
    CoraokeConfig.screensaverTimeout = v;
  },
};
...
const setupSocketEvents = () => {
  ...
  socket.on('connect_error', (error) => {
    ...
    flashNotification(CoraokeConfig.translations.socketConnectionLost, "is-danger");
  });
...
$(function () {
  ...
  setupUIScaling();
  if (CoraokeConfig.showSplashClock) startClock();
  setupScreensaver();
  setupOverlayMenus();
  setupVideoPlayer();
