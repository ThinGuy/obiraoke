...
class TestQueueReorderSocketUpdates:
    ...
    @patch("obiraoke.routes.queue.is_admin", return_value=True)
    @patch("obiraoke.routes.queue.get_karaoke_instance")
    @patch("obiraoke.routes.queue.broadcast_event")
    @patch("obiraoke.routes.queue._", side_effect=lambda x: x)
