...
class TestQueueReorderSocketUpdates:
    ...
    @patch("coraoke.routes.queue.is_admin", return_value=True)
    @patch("coraoke.routes.queue.get_karaoke_instance")
    @patch("coraoke.routes.queue.broadcast_event")
    @patch("coraoke.routes.queue._", side_effect=lambda x: x)
